...
def structDIE_to_varlib(sdie:DIE, name:str, is_class:bool=False):
    ...
    if sid == -1:
        ...
        stype = StructType(sid, _struct_db)                             # 2. NOW define fields (after mapping to prevent recursion issues)
        stype.layout = get_layout_from_structDIE(sdie)
        return stype
    else:
        stype = StructType(sid, _struct_db)     # get existing type from sid
