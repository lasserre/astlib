...
class ASTViewer(VisitAllChildrenByDefaultVisitor):
    ...
    def render_ast(self, node:'ASTNode', format:str, outfolder:Path=None, ast_name:str='',
        ...
        self.clear_ids_in_tree(node)

        return self.g

    def clear_ids_in_tree(self, root:ASTNode):
        nodes = [root]
        while len(nodes):
            n = nodes.pop()
            if ASTViewer.is_render_node(n):
                delattr(n, '_graph_id')
            nodes.extend(n.inner)
    ...
    def create_graph_node(self, node:'ASTNode', parent:'ASTNode', attrs:NodeAttrs):
        ...
        if parent and ASTViewer.is_render_node(parent):
    ...
    def is_render_node(node:ASTNode) -> bool:
        return '_graph_id' in node.__dict__
